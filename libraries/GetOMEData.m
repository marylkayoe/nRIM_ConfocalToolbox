...
% Get OME Meta Information using BioFormats Library
    ...
    try
        for id = 1:imagecount
            ...
            imageIDs{id} = id-1; 
            [OMEData.ImageCenterX(id) OMEData.ImageCenterY(id)] = LF_getPositionValues(omeMeta, imageIDs{id});
        end
        % store ine OMEData
...
function [x y] = LF_getPositionValues(omeMeta, imageID)

% Get the string representation of the OME metadata value
aa = char(omeMeta.getPlanePositionX(imageID, 1));

% Extract the numerical value from the string using regular expressions
expr = '-?\d+\.?\d*';  % regular expression to match decimal numbers
tokens = regexp(aa, expr, 'match');
if ~isempty(tokens)
    x = str2double(tokens{1});  % convert the first token to a double
else
    error('Failed to extract numerical value from string: %s', aa);
end

aa = char(omeMeta.getPlanePositionY(imageID, 1));

% Extract the numerical value from the string using regular expressions
expr = '-?\d+\.?\d*';  % regular expression to match decimal numbers
tokens = regexp(aa, expr, 'match');
if ~isempty(tokens)
    y = str2double(tokens{1});  % convert the first token to a double
else
    error('Failed to extract numerical value from string: %s', aa);
end
end
